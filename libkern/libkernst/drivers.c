...
extern EFI_STATUS Draw_Window_Error(EFI_GRAPHICS_OUTPUT_PROTOCOL GraphicsOutput);
extern UINTN Index;
extern UINTN Pixels;
extern UINT32 y;
...
EFI_STATUS ExecuteMachoKernel(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable, CHAR16* FilePath) {
	...
	if (StrCmp(Buffer, L"machokrnl") == 0 && LineIndex == 0) {
		...
		Print(L"\nMach-O header not confirmed\n");
		break;
	} else if (StrCmp(Buffer, L"_CreateStartFile") == 0) {
		CHAR8 *Text = "Macho Kernel Started";
    		UINTN Length = 20;
		
    		Status = Create_File(ImageHandle, SystemTable, L"\\tmp\\log1.tmp", Length, Text, 0);	
	} else if (StrCmp(Buffer, L"PEAASX$%$#^%@%$2") == 0) {
		Print(L"Welcome to Mach-O Kernel!\n");
	} else if (StrCmp(Buffer, L"BootRescueMode33") == 0) {
		Status = ExecuteCommand(Status, ImageHandle, SystemTable);
	} else if (StrCmp(Buffer, L"_KrnlClearScreen") == 0) {
		...
		Print(L"\nUnknown Command\nBuffer: %s\n", Buffer);
		Status = Draw_Window_Error(*GraphicsOutput);
		Print_Unicode(*GraphicsOutput, "!!!!Unknown Instruction!!!!", 640, 380, 0xffffffff);
		break;
	}
	LineIndex++;
    }
